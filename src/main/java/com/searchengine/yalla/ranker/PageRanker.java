package com.searchengine.yalla.ranker;
...
public class PageRanker {
	...
	public ArrayList<String> pageRelevance(ArrayList<String> searchWords, Boolean isPhrase) throws SQLException{
		...
		for(int i = 0; i < docsCount; i++){
			...
			while(words_counts.next())  {
				...
				if(word_h1.get(curr_doc_id, curr_word_id) != null){
					curr_doc_score += (int)word_h1.get(curr_doc_id, curr_word_id) * 0.2;
				}
		...
		Map<String, Float> urls = new HashMap<>();
		
		for(int i =0; i<docsCount; i++) {
			urls.put(docUrl.get(docIds.get(i)), docScore.get(docIds.get(i)));
		}
		
		Map<String, Float> sorted = urls
	            .entrySet()
	            .stream()
		...
		/*
		for(int i=0; i<docsCount; i++) {
			System.out.println(docs.getInt(1) + "  " + docScore.get(docs.getInt(1)));
			docs.next();
		}
		*/
	...
	public ArrayList<String> imageRelevance(ArrayList<String> searchWords) throws SQLException{
		ArrayList<String> ImageUrls = new ArrayList<String>();
		ResultSet imgs = MySqlTest.Select_Imgs(searchWords);
		while(imgs.next())
			ImageUrls.add(imgs.getString(1));
	...
	public static void main(String[] args) throws SQLException {
		...
		//p1.pageRelevance(searchWords,true);
