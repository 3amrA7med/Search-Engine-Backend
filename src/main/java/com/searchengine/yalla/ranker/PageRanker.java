...
public class PageRanker {
	...
	public List<Object> pageRelevance(ArrayList<String> searchWords, Boolean isPhrase
		...
		if(sortedUrls.size() == 0) {
			wantedUrls = new ArrayList<Integer>();
		}
		else {
			if(length >= (sortedUrls.size() - offset)) {
				wantedUrls = new ArrayList<Integer>(sortedUrls.subList(offset, sortedUrls.size()));
			}else {
				wantedUrls = new ArrayList<Integer>(sortedUrls.subList(offset, offset+length));
			}
		}
